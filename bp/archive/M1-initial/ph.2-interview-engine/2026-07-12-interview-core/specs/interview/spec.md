# Delta-Spec: interview

> Change: interview-core | Domain: interview (NEW)
...
<!-- interview-core adds new behavior without modifying pre-existing global specs.
The interview domain spec is the source of truth for state machine, scoring, and
`mi interview` command surface. The existing global specs are unaffected because:
 - specs/storage/spec.md — already documents interviews + interview_answers tables
   via the `database-migration` change; schema is unchanged here
 - specs/cli-config/spec.md — interview commands follow the same conventions as
   existing init / config commands (Chinese output, exit codes 1/2, --json flag,
   runCommandAction wrapper) without adding new framework rules
 - specs/core/spec.md — generic input validation applies; interview-specific
   validation scenarios live in this domain spec
No MODIFIED requirements are emitted in this delta. -->
...
<!-- interview-core adds new interview behavior without removing any pre-existing contract. -->
